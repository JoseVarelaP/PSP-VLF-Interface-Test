...
int OnLogin(int enter)
{
	...
	else
	{
		...
		vlfGuiSetRectangleFade(0, VLF_TITLEBAR_HEIGHT, 480, 272-VLF_TITLEBAR_HEIGHT, VLF_FADE_MODE_OUT, VLF_FADE_SPEED_SUPER_FAST, 0, BackToTitleMenu, NULL, 0);
	}
...
void OnLicenseAgreementFadeOutComplete(void *param)
{
	...
	rp_label = vlfGuiAddText(95, 153, rp == NULL ? "CHECKMARK NULL" : "Remember password");
...
void StartSample()
{
	...
	vlfGuiSetRectangleFade(0, VLF_TITLEBAR_HEIGHT, 480, 272-VLF_TITLEBAR_HEIGHT, VLF_FADE_MODE_IN, VLF_FADE_SPEED_STANDARD, 0, NULL, NULL, 0);
}
...
void MainMenu()
{
	logo = vlfGuiAddPictureFile("logo.png", 144, 80); // png's are now supported
	
	const char *items[] = { "Begin Test", "Exit" };
	// vlfGuiLateralMenu(2, items, 0, menu_sel, 100);
	vlfGuiCentralMenu(2, items, 0, menu_sel, 0, 80);
	vlfGuiSetTitleBar(header_text, NULL, 1, 0);
...
{	
	...
	header_text = vlfGuiAddText(0,0,"VLF-Lib Testing");
